...
nn$Lat <- rep(melt(mi31[,,"2014"])[,1], 18)
nn$Lon <- rep(melt(mi31[,,"2014"])[,2], 18)
nn$Year <- rep(c(1931,1936,1956,1961,2002,2014), each = 26026*3 )
nn$Period <- rep(c("a","b","c"), each = 26026)

ggplot(nn, aes(x = Lat, y = Lon, z = value, fill = value)) + geom_tile() + coord_equal() +  scale_fill_distiller(palette="Spectral", na.value="white") + borders("state", xlim = c(-124.6875, -102.0625), ylim = c(31.1875, 48.9375)) + facet_grid(Period~Year) + theme_minimal()
...
all_var$mi31 <- NA
all_var[624625:30294264,"mi31"] <- as.list(aperm(apply(mi31, 1:2, function(x) rep(x, each = 12)), c(2,3,1))) # from 1920 to 2014, the full dataset goes from 1918 to 2014
all_var[624625:30294264,"mi31"]
all_var$mi31r <- all_var$mi31 * all_var$md00ygs

#apply mean to md00 by year
aggregate(mi31r ~ year, data = all_var, mean, na.rm = T)
